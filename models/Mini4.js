...
export default class Mini4 {
  ...
  static getDiagnosis(allEquips, totalScores) {
    ...
    const 耐風最高速 =
      Math.max(
        総合スピード * (総合パワー - 耐風負荷) - エアロダウンフォース / 1000,
        最高速度ms / 5
      ) * 3.6
    ...
    const 芝最高速 =
      Math.max(
        総合スピード * (総合パワー - 芝負荷) - エアロダウンフォース / 1000,
        最高速度ms / 5
      ) * 3.6
    ...
    const ダート最高速 =
      Math.max(
        総合スピード * (総合パワー - ダート負荷) - エアロダウンフォース / 1000,
        最高速度ms / 5
      ) * 3.6
    ...
    const フロントリヤタイヤ旋回 = リヤタイヤ旋回 + フロントタイヤ旋回
    const フロントリヤタイヤ旋回差 =
